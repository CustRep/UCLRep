...
public:
  ImageConverter()
    : it_(nh_), pixel_x(0),pixel_y(0), min_distence(1.0),
      z_scale_(1.0), x_scale_(5.0)
  {
    ...
    pixel_cmdpub_ = nh_.advertise<geometry_msgs::Twist> ("final/cmd_vel", 1);
  ...
  */
  ...
  void drawObject(int x,int y,Mat &frame)
  {
    ...
    // debug the pixel_x and pixel_y 

    std::cout << "pixel_x :" << intToString(pixel_x) << " pixel_y : " << intToString(pixel_y) << std::endl;
    ...
    if(x < FRAME_WIDTH/2 + FORWARD_TUNE && x > FRAME_WIDTH/2 - FORWARD_TUNE)
    {
      ...
      //ss << "fw";
      geometry_msgs::TwistPtr fw_cmd(new geometry_msgs::Twist());
      fw_cmd->linear.x = 0.35;
      // cmd->angular.z = -x * x_scale_;
      // cmdpub_ -> /cmd_vel,  pixel_cmdpub_ -> /final/cmd_vel
      cmdpub_.publish(fw_cmd);
    }
    else if(x > FRAME_WIDTH/2 + 30)
    {
      std::cout << "Turning Left" << std::endl;
      //ss << "left";
      geometry_msgs::TwistPtr left_cmd(new geometry_msgs::Twist());
      left_cmd->angular.z = 0.25;
      cmdpub_.publish(left_cmd);
    }
    else
    {
      std::cout << "Turning Right" << std::endl;
      //ss << "right";
      geometry_msgs::TwistPtr right_cmd(new geometry_msgs::Twist());
      right_cmd->angular.z = -0.25;
      cmdpub_.publish(right_cmd);
    }
    //float angle_f = (x-320)/10.66;
    float angle_f = (x-640)/10.66;
  ...
  void depthImageCb(const sensor_msgs::ImageConstPtr& depth_msg)
  {
    ...
    for (int v = 0; v < (int)depth_msg->height; ++v, depth_row += row_step)
    {
     for (int u = 0; u < (int)depth_msg->width; ++u)
     {
       ...
       if ( y_val > (pixel_y-10) && y_val < (pixel_y+10) &&
            x_val > (pixel_x-10) && x_val < (pixel_x+10))
       {
         x += x_val;
         y += y_val;
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    morphOps(threshold);
    // After Morphological Operations image
